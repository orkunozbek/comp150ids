...
// Convertes __int[10][10] to __int_10_10
// For function name generation and function call
...
// Creates a function for converting structs to bytes and vice versa for network transfer
// This data conversion conforms to our data storage protocol
...
// Creates a function for getting sizeof structs
...
// Creates a function for getting sizeof array types
...
// Creates a function for converting array types to bytes and vice versa for network transfer
// This data conversion conforms to our data storage protocol
...
int main(int argc, char *argv[]){
    ...
    // Read the templates -- proxy, structsize, structconversion, arraysize, arrayconversion
	...
	for (argnum = 1; argnum < argc; argnum++) {
        ...
        conversionFunctionHeaders.clear();
      	...
      	std::map<std::string, TypeDeclaration*>::iterator iter;  // Get types
		TypeDeclaration *typep;
		stringstream formattedType;
		
		// Iterate through types -- create conversions and size functions
		...
		// Iterate through functions -- create proxy functions
				...
				string strNum(strBuffer);
	  			if(typeNameStr == "int"){
	  				...
	  				if(typep->isStruct()){
	  					...
	  					argumentConversions.append("\tchar* arrayData" + strNum + " = (char*)convert" + arrayTypeNameHelper(typep) + "ToByte(" + argNameStr + ",\"" + argNameStr + "\", NULL);\n");
      	...
      	// Replace the parts of the proxy template with the generated code
